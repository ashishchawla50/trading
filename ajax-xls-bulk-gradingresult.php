...
if(isset($_FILES["Upload_file_gradingresult"]))
{
	

	$XLSPATH = isset($_FILES["Upload_file_gradingresult"]["tmp_name"]) ? $_FILES["Upload_file_gradingresult"]["tmp_name"] : "";
	$Path = UPLOAD."xls/GRADINGRESULT/";
	$XLSPATH = $Path."GRADINGRESULT.xls";
	move_uploaded_file($_FILES["Upload_file_gradingresult"]["tmp_name"],$XLSPATH);
	...
	for($i=2;$i<=count($row);$i++)
	{
		...
		$LAB = isset($row[$i][2])? $row[$i][2]:'';
		$CERTIFICATENO = isset($row[$i][3])? $row[$i][3]:'';
		$SHAPE = isset($row[$i][4])? $row[$i][4]:'';
		$WEIGHT = isset($row[$i][5])? $row[$i][5]:'';
		$COLOR = isset($row[$i][6])? $row[$i][6]:'';
		$CLARITY = isset($row[$i][7])? $row[$i][7]:'';
		$CLARITY = str_replace(" ","",$CLARITY);
		$CUT = isset($row[$i][8])? $row[$i][8]:'';
		$POLISH = isset($row[$i][9])? $row[$i][9]:'';
		$SYMM = isset($row[$i][10])? $row[$i][10]:'';
		$FLOURANCE = isset($row[$i][11])? $row[$i][11]:'';
		
		
		$RAPRATE = isset($row[$i][15])? $row[$i][15]:'';
		
		$BGM = isset($row[$i][16])? $row[$i][16]:'';
		if(empty($RAPRATE))
		{
		...
		$DISC3PER = isset($row[$i][14])? $row[$i][14]:'';
		if($DISC3PER > 0)
		{
