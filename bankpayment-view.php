...
if(isset($_POST["SRNO"]) && isset($_POST["VOUCHERTYPE"]) && isset($_POST["tran-bankpayment"]))
{
...
$AMOUNT =$_POST["txtCRAMOUNT"];
	...
	?>
	<script>
		window.location.href='<?php echo SITEURL; ?>?tran-bankpayment-v';
	</script>
	<?php
	exit();
}
else
{
	...
	if(isset($_GET["_mid"]))
	{
		...
		$res = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Bank Payment' AND DR.VOUCHERTYPE='Bank Payment' AND DR.SRNO='".$SRNO."' AND CR.SRNO='".$SRNO."'");
...
//$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Bank Payment' AND DR.VOUCHERTYPE='Bank Payment' ORDER BY DR.VOUCHERDATE DESC,DR.SRNO ASC");
$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Bank Payment' AND DR.VOUCHERTYPE='Bank Payment' ORDER BY DR.SRNO DESC");

?>
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Bank - Payment - Voucher - View</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
...
<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?tran-bankpayment-v" method="POST" onsubmit="return confirm('Do you really want to Delete?');">
								...
								while($resdata = mysqli_fetch_assoc($res_dr))
										{
											...
											?>
												...
												<td class="delcls">
													...
													{
														?>
														<a href="<?php echo SITEURL; ?>?bankpayment-view&_mid=<?php echo $resdata["SRNO"];?>" class="btn btn-primary btn-circle" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?bankpayment-view&_rid=<?php echo $resdata["SRNO"];?>" onclick="return confirm('Do you really want to Delete This Transaction : <?php echo $resdata["SRNO"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
...
{
	$moneycol= "";
	if($resdata["AMOUNTDOLLAR"] > 0 && $resdata["RMBAMOUNT"] == 0 && $resdata["CRAMOUNT"] ==0)
	{
		$moneycol = "$";
	}
	elseif($resdata["AMOUNTDOLLAR"] == 0 && $resdata["RMBAMOUNT"] > 0 && $resdata["CRAMOUNT"] ==0)
	{
		$moneycol = "RMB";
	}
	elseif($resdata["AMOUNTDOLLAR"] == 0 && $resdata["RMBAMOUNT"] == 0 && $resdata["CRAMOUNT"] >0)
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				<div class="panel-heading">
                    Bank - Payment - Voucher - New
                </div>
				<div class="panel-body">
					<p>
					<?php
						echo '
								<a  class="btn btn-warning" href="'.SITEURL."?".$filename.'" style="float:right;margin-left:10px;" ><i class="fa fa-backward"></i> Back</a>
								
							';
						?>
						<a  class="btn btn-success" href="<?php echo SITEURL; ?>?bankpayment-view" style="float:right;" ><i class="fa fa-tasks"></i> View All</a>
						<br>
					</p>
					<form id="frm_journaltable_edit" action="<?php echo SITEURL; ?>?tran-bankpayment-v" method="POST" >
						...
						<div class="form-group">
							<table width="100%" class="inputfieldtable">
								...
								<tr>
									<td>
										<input type="text" class="form-control onlyNumber amtchange_mid" name="txtAMOUNTDOLLAR" id="txtCRAMOUNTDOLLAR" value="<?php echo $resdata["AMOUNTDOLLAR"]?>">
									</td>
									<td>
									...
									<td>
										...
										<input type="text" class="form-control onlyNumber amtchange_mid" name="txtRMBAMOUNT" id="txtCRRMBAMOUNT" value="<?php echo $resdata["RMBAMOUNT"]?>">
									...
									<td>
										<input type="text" class="form-control onlyNumber amtchange_mid CRAMOUNT" name="txtCRAMOUNT" id="txtCRAMOUNT" value="<?php echo $resdata["CRAMOUNT"]?>" >
									</td>
						...
						<button type="submit" class="btn btn-default" style="float: right;" name="tran-bankpayment">Submit Button</button>
					</form>
				</div>
			</div>
		</div>
