<?php
if(isset($_POST["tran-bankreceipt"]))
{
		$idarr = array();
	$idarr = $_POST["radID"];
	...
	if(count($idarr) == 1 )
	{
		...
		$VOUCHERTYPE = $_POST["txtVOUCHERTYPE".$VOUCHERNO];
	...
	else
	{
		...
		<script>
			...
			window.location.href="<?php echo SITEURL."?tran-bankreceipt";?>";
		</script>
		<?php
		exit();
}
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Bank - Receipt - Voucher</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					...
					?>
					<p>
						<a  class="btn btn-warning" href="<?php echo SITEURL; ?>?bankreceipt-view" style="float:right;" ><i class="fa fa-tasks"></i> View 
All</a>
						<br>
					</p>
					<?php
					...
					?>
					<form id="tran-bankreceipt" action="<?php echo SITEURL; ?>?tran-bankreceipt" method="POST">
...
<div class="form-group">
							<table width="100%" class="inputfieldtable">
								<tr>
									<td><label>Book Name</label> <a href="javascript:void(0)" class="LEDGER_auto" rel="1" ><i class="fa fa-plus-circle"></i> 
Add New</a></td>
								...
								<tr>
									...
									<select class="form-control" name="MONEYCOL" id="MONEYCOL">
												<option value="₹" <?php echo isset($_POST["MONEYCOL"]) && $_POST["MONEYCOL"] == '₹' ? 'selected="selected"' : 
''?>>₹</option>
												<option value="$" <?php echo isset($_POST["MONEYCOL"]) && $_POST["MONEYCOL"] == '$' ? 'selected="selected"' : 
''?>>$</option>
												<option value="RMB" <?php echo isset($_POST["MONEYCOL"]) && $_POST["MONEYCOL"] == 'RMB' ? 
'selected="selected"' : ''?>>RMB</option>
												<option value="$-₹" <?php echo isset($_POST["MONEYCOL"]) && $_POST["MONEYCOL"] == '$-₹' ? 
