...
if(isset($_POST["SRNO"]) && isset($_POST["VOUCHERTYPE"]) && isset($_POST["tran-cash"]))
{
	...
	if($_POST["txtCRAMOUNT"] == $_POST["txtDRAMOUNT"])
	{
		$CRAMT = $_POST["txtCRAMOUNT"];
		$DRAMT = $_POST["txtDRAMOUNT"];
	}
	elseif($_POST["txtCRAMOUNT"] > $_POST["txtDRAMOUNT"])
	{
		$CRAMT = $_POST["txtCRAMOUNT"];
		$DRAMT = $_POST["txtGRANDAMOUNT"];
	}
	elseif($_POST["txtCRAMOUNT"] < $_POST["txtDRAMOUNT"])
	{
		$CRAMT = $_POST["txtGRANDAMOUNT"];
		$DRAMT = $_POST["txtCRAMOUNT"];
	}
	...
	print_r($tranFieldArr);
	print_r($tranValueArr);
	exit();
	...
	?>
	<script>
		window.location.href='<?php echo SITEURL; ?>?tran-cash-v';
	</script>
	<?php
	exit();
}
else
{
	...
	if(isset($_GET["_mid"]))
	{
		...
		$Caption = "Edit Cash Voucher Detail";
		$res = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Cash Payment' AND DR.VOUCHERTYPE='Cash Payment' AND DR.SRNO='".$SRNO."' AND CR.SRNO='".$SRNO."'");
		...
		$CR_AMT = getFieldDetail(LEDGER_CREDIT, "SUM(AMOUNT)", " WHERE LEDGERID='".$resdata["DRLEDGERID"]."' AND VOUCHERDATE <='".$dtfrm."' AND VOUCHERTYPE IN ('Journal','Cash Payment','Bank','Opening')");
		$DR_AMT = getFieldDetail(LEDGER_DEBIT, "SUM(AMOUNT)", " WHERE LEDGERID='".$resdata["DRLEDGERID"]."' AND VOUCHERDATE <='".$dtfrm."' AND VOUCHERTYPE IN ('Journal','Cash Payment','Bank','Opening')");
		$BAL_DR = (($DR_AMT-$CR_AMT)) > 0 ? (($DR_AMT-$CR_AMT)) : Abs(($DR_AMT-$CR_AMT));
	
		
		$CR_AMT = getFieldDetail(LEDGER_CREDIT, "SUM(AMOUNT)", " WHERE LEDGERID='".$resdata["CRLEDGERID"]."' AND VOUCHERDATE <='".$dtfrm."' AND VOUCHERTYPE IN ('Journal','Cash Payment','Bank','Opening')");
		$DR_AMT = getFieldDetail(LEDGER_DEBIT, "SUM(AMOUNT)", " WHERE LEDGERID='".$resdata["CRLEDGERID"]."' AND VOUCHERDATE <='".$dtfrm."' AND VOUCHERTYPE IN ('Journal','Cash Payment','Bank','Opening')");
...
$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Cash Payment' AND DR.VOUCHERTYPE='Cash Payment'");

?>
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Cash - Voucher - View</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
...
<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?tran-cash-v" method="POST" onsubmit="return confirm('Do you really want to Delete?');">
					...
					<div class="dataTable_wrapper">
						<table class="table table-striped table-bordered table-hover" id="dataTables-example">
							<thead>
								<tr>
									<td colspan="5"></td>
									<td colspan="2" style="text-align:center;font-size:1.2em;"><label>Debit</label></td>
									<td colspan="2" style="text-align:center;font-size:1.2em;"><label>Credit</label></td>
									<td colspan="1"></td>
								</tr>
								<tr>
								...
								while($resdata = mysqli_fetch_assoc($res_dr))
										{
											...
											?>
												...
												<td class="delcls">
													<a href="<?php echo SITEURL; ?>?tran-cash-v&_mid=<?php echo $resdata["SRNO"];?>" class="btn btn-primary btn-circle" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?tran-cash-v&_rid=<?php echo $resdata["SRNO"];?>" onclick="return confirm('Do you really want to Delete This Transaction : <?php echo $resdata["SRNO"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<p>
						<a  class="btn btn-warning" href="<?php echo SITEURL; ?>?tran-cash-v" style="float:right;" ><i class="fa fa-tasks"></i> View All</a>
						<br>
					</p>
					<form id="frm_journaltable_edit" action="<?php echo SITEURL; ?>?tran-cash-v" method="POST" >
						...
						<div class="form-group">
							<table width="60%" class="inputfieldtable">
								...
								<tr>
									<td>
										<select class="form-control" name="txtDRLEDGERID" id="txtBOOKLEDGERID">
											<?php
									...
									<td>
										 <select class="form-control" name="txtCRLEDGERID" id="txtLEDGERID">
											<?php
											$res_led = getData(LEDGER,$AllArr," WHERE FLAG='0'");
											while($res_led_data = mysqli_fetch_assoc($res_led))
												{
						...
						<div class="form-group">
							<table width="100%" class="inputfieldtable">
								...
								<tr>
									<td>
										<input type="text" class="form-control onlyNumber amtchange DRAMOUNT" name="txtDRAMOUNT" id="txtDRAMOUNT" value="<?php echo $resdata["DRAMOUNT"]?>" >
									</td>
									<td>
										<input type="text" class="form-control onlyNumber amtchange CRAMOUNT" name="txtCRAMOUNT" id="txtCRAMOUNT" value="<?php echo $resdata["CRAMOUNT"]?>" >
									</td>
									<td>
										<input type="text" class="form-control onlyNumber amtchange" name="txtCONVRATE" id="txtCONVRATE" value="<?php echo $resdata["CONVRATE"]?>">
									</td>
									<td>
										<input type="text" class="form-control onlyNumber" name="txtAMOUNTDOLLAR" id="txtAMOUNTDOLLAR" value="<?php echo $resdata["AMOUNTDOLLAR"]?>">
									</td>
								</tr>
						...
						<div class="form-group">
							<table width="100%" class="inputfieldtable">
								...
								<tr>
									<td>
										<input type="text" class="form-control onlyNumber gstchange CRDRAMOUNT" name="txtIGSTAMT" id="txtIGSTAMT" value="<?php echo $IGSTAMT;?>" >
									</td>
									<td>
										<input type="text" class="form-control onlyNumber gstchange CRDRAMOUNT" name="txtCGSTAMT" id="txtCGSTAMT" value="<?php echo $CGSTAMT;?>">
									</td>
									<td>
										<input type="text" class="form-control onlyNumber gstchange CRDRAMOUNT" name="txtSGSTAMT" id="txtSGSTAMT" value="<?php echo $SGSTAMT;?>">
									</td>
									<td>
										<input type="text" class="form-control onlyNumber" name="txtGRANDAMOUNT" id="txtGRANDAMOUNT" value="<?php echo $GRANDAMOUNT;?>">
									</td>
								</tr>
						...
						<button type="submit" class="btn btn-default" style="float: right;" name="tran-cash">Submit Button</button>
					</form>
				</div>
			</div>
		</div>
