<?php
	$FieldArr= array();
								...
								array_push($FieldArr,"DISC2PER");
								array_push($FieldArr,"DISC3PER");
		...
		$res = getData(BARCODE_PROCESS,$FieldArr," AS BP INNER JOIN ".PURCHASESALE." AS PS ON PS.ID=BP.ID AND PS.VOUCHERTYPE=BP.PROCESSTYPE INNER JOIN ".LEDGER." AS L ON L.LEDGERID=BP.LEDGERID LEFT JOIN ".LEDGER." AS B on B.LEDGERID=BP.BROKERID WHERE BP.FLAG='0' AND BP.PROCESSTYPE='Purchase'" .$BARCODENOSEARCH." ORDER BY PS.VOUCHERDATE desc");
...
if(isset($_GET["_nid"]))
{
	...
	if(!is_array($resdata))
	{
		...
		$resdata["LEDGERID"]='';
		$resdata["BROKERID"]='';
		$resdata["LOCATIONNAME"]='';
		$resdata["CONVRATE"]='';
		$resdata["RMBSTATUS"]='';
		$resdata["TOTALQTY"]=0;
		$resdata["PERCRTTOTALDOLLAR"]=0;
		$resdata["TOTALDOLLAR"]=0;
		$resdata["TOTALEXPENCE"]=0;
		$resdata["TOTALRSRSPERCRT"]=0;
		$resdata["TOTALRSAMOUNT"]=0;
		$resdata["DALALIPER"]=0;
		$resdata["DALALIAMT"]=0;
		$resdata["FINALTOTAL"]=0;
		$resdata["BILLSTATUS"]='';
		$resdata["INVOICECHAR"]='';
		$resdata["RMBRATE"]=0;
		$resdata["IGSTPER"]=0;
		...
		$resdata["CGSTPER"]=0;
		$resdata["CGSTAMT"]=0;
		$resdata["LASTAMOUNT"]=0;
		$resdata["GRANDAMOUNT"]=0;
		$resdata["TCSAMT"]=0;
		$resdata["TCSPER"]=0;
		$resdata["REMARK"]='';
		$resdata["CONVSTATUS"]='';
		$resdata["PARTNERPER"]='';
		$resdata["THIRDPARTYCHARGES"]='';
		$resdata["THIRDPARTYCHARGESPER"]='';
		$resdata["THIRDPARTYTCSPER"]='';
		$resdata["THIRDPARTYTCS"]='';
		$resdata["TOTALRMBPERCRT"]='';
		$resdata["TOTALRMBAMOUNT"]='';
		$resdata["PARTNERID"]='';
		$resdata["PARTNERAMOUNT"]='';
		$resdata["THIREDPERID"]='';
		$resdata["COMMCHARGE"]='';
		$resdata["COMMDISC1PER"]='';
		$resdata["COMMDISC2PER"]='';
		$resdata["TOTALCOMM"]='';
...
elseif(isset($_GET["_mid"]))
{
	...
	$res = getData(PURCHASESALE,$AllArr," WHERE ID='".$ID."' AND VOUCHERTYPE='Purchase' ORDER BY VOUCHERDATE DESC");
	$resdata = mysqli_fetch_assoc($res);	
...
elseif(isset($_GET["_rid"]))
{
	...
	deleteData(LEDGER_DEBIT," WHERE VOUCHERNO ='".$ID."' AND VOUCHERTYPE in ('Purchase','Third Party Charges','Tax In','TCS In')");
	...
	?>
	<script>
		window.location.href="<?php echo SITEURL."?purchase&_vid";?>";
	</script>
	<?php
}
...
if(isset($_POST["purchase"]))
{
	...
	$cnt_3party = getFieldDetail(LEDGER_DEBIT,"count(*)"," WHERE VOUCHERNO='". $ID ."' AND VOUCHERTYPE='Third Party Charges' AND LEDGERID='" . THIRDPARTYCHARGESAC . "'");
	$cnt_3partytcs = getFieldDetail(LEDGER_DEBIT,"count(*)"," WHERE VOUCHERNO='". $ID ."' AND VOUCHERTYPE='Third Party TCS' AND LEDGERID='" . THIRDPARTYTCSAC . "'");
	...
	$cnt_tcs = getFieldDetail(LEDGER_DEBIT,"count(*)"," WHERE VOUCHERNO='". $ID ."' AND VOUCHERTYPE='TCS In' AND LEDGERID='" . TCSIN . "'");
	if ($reccnt == 0)
		{
	...
	foreach ($Barcode_Arr as $Barcode)
	{
		...
		if ($reccnt == 0)
		{
			...
			array_push($FieldBarcode,"THIRDPARTYCHARGESPER");
			array_push($ValueBarcode,"'". $_POST["txtTHIRDPARTYCHARGESPER"] ."'");
			array_push($FieldBarcode,"THIRDPARTYTCSPER");
			array_push($ValueBarcode,"'". $_POST["txtTHIRDPARTYTCSPER"] ."'");
		...
		if($_POST["txtTHIRDPARTYCHARGES"] > 0 &&  (isset($_POST["radBILLSTATUS"]) && $_POST["radBILLSTATUS"]=='Third Party Bill'))
		{
			$tran3PARTYFieldArr= array();
			$tran3PARTYValueArr= array();
			array_push($tran3PARTYFieldArr,"SRNO");
			array_push($tran3PARTYFieldArr,"VOUCHERNO");
			array_push($tran3PARTYFieldArr,"VOUCHERTYPE");
			array_push($tran3PARTYFieldArr,"LEDGERID");
			array_push($tran3PARTYFieldArr,"AMOUNT");
			array_push($tran3PARTYFieldArr,"DESCRIPTION");
			array_push($tran3PARTYFieldArr,"VOUCHERDATE");
			array_push($tran3PARTYFieldArr,"UPDATEDATE");
			array_push($tran3PARTYFieldArr,"USERNAME");
			array_push($tran3PARTYFieldArr,"CONVRATE");
			array_push($tran3PARTYFieldArr,"AMOUNTDOLLAR");
			array_push($tran3PARTYFieldArr,"GROUPID");
			array_push($tran3PARTYFieldArr,"BILLSTATUS");
			
			array_push($tran3PARTYValueArr,"'".$SRNO."'");
			array_push($tran3PARTYValueArr,"'".$ID."'");
			array_push($tran3PARTYValueArr,"'Third Party Charges'");
			array_push($tran3PARTYValueArr,"'".THIRDPARTYCHARGESAC."'");
			array_push($tran3PARTYValueArr,"'".$_POST["txtTHIRDPARTYCHARGES"]."'");
			array_push($tran3PARTYValueArr,"'".$_POST["txtREMARK"]."'");
			array_push($tran3PARTYValueArr,"'".$_POST["dtpVOUCHERDATE"]."'");
			array_push($tran3PARTYValueArr,"'".date('Y-m-d h:i:s')."'");
			array_push($tran3PARTYValueArr,"'".$loginuser_name."'");
			array_push($tran3PARTYValueArr,"''");
			array_push($tran3PARTYValueArr,"''");
			array_push($tran3PARTYValueArr,"'".THIRDPARTYCHARGESGB."'");
			array_push($tran3PARTYValueArr,"'".(isset($_POST["radBILLSTATUS"]) ? $_POST["radBILLSTATUS"] : '')."'");
			
			if ($cnt_3party == 0)
			{
				array_push($tran3PARTYFieldArr,"ENTRYDATE");
				array_push($tran3PARTYValueArr,"'".date('Y-m-d h:i:s')."'");			
				newData($tran3PARTYFieldArr,$tran3PARTYValueArr,LEDGER_DEBIT);		
			}
			else
			{
				$Condition = " WHERE LEDGERID='".THIRDPARTYCHARGESAC."' AND VOUCHERTYPE='Third Party Charges' AND SRNO='".$SRNO."' AND VOUCHERNO='".$ID."'";			
				editData($tran3PARTYFieldArr,$tran3PARTYValueArr,LEDGER_DEBIT,$Condition);			
			}	
		}
		else
		{
		...
		#-------------------------THIRD PARTY TCS---------------------------
		if($_POST["txtTHIRDPARTYTCS"] > 0 &&  (isset($_POST["radBILLSTATUS"]) && $_POST["radBILLSTATUS"]=='Third Party Bill'))
		{
			$tran3PARTYTCSFieldArr= array();
			$tran3PARTYTCSValueArr= array();
			array_push($tran3PARTYTCSFieldArr,"SRNO");
			array_push($tran3PARTYTCSFieldArr,"VOUCHERNO");
			array_push($tran3PARTYTCSFieldArr,"VOUCHERTYPE");
			array_push($tran3PARTYTCSFieldArr,"LEDGERID");
			array_push($tran3PARTYTCSFieldArr,"AMOUNT");
			array_push($tran3PARTYTCSFieldArr,"DESCRIPTION");
			array_push($tran3PARTYTCSFieldArr,"VOUCHERDATE");
			array_push($tran3PARTYTCSFieldArr,"UPDATEDATE");
			array_push($tran3PARTYTCSFieldArr,"USERNAME");
			array_push($tran3PARTYTCSFieldArr,"CONVRATE");
			array_push($tran3PARTYTCSFieldArr,"AMOUNTDOLLAR");
			array_push($tran3PARTYTCSFieldArr,"GROUPID");
			array_push($tran3PARTYTCSFieldArr,"BILLSTATUS");
			
			array_push($tran3PARTYTCSValueArr,"'".$SRNO."'");
			array_push($tran3PARTYTCSValueArr,"'".$ID."'");
			array_push($tran3PARTYTCSValueArr,"'Third Party TCS'");
			array_push($tran3PARTYTCSValueArr,"'".THIRDPARTYTCSAC."'");
			array_push($tran3PARTYTCSValueArr,"'".$_POST["txtTHIRDPARTYTCS"]."'");
			array_push($tran3PARTYTCSValueArr,"'".$_POST["txtREMARK"]."'");
			array_push($tran3PARTYTCSValueArr,"'".$_POST["dtpVOUCHERDATE"]."'");
			array_push($tran3PARTYTCSValueArr,"'".date('Y-m-d h:i:s')."'");
			array_push($tran3PARTYTCSValueArr,"'".$loginuser_name."'");
			array_push($tran3PARTYTCSValueArr,"''");
			array_push($tran3PARTYTCSValueArr,"''");
			array_push($tran3PARTYTCSValueArr,"'".THIRDPARTYTCSGB."'");
			array_push($tran3PARTYTCSValueArr,"'".(isset($_POST["radBILLSTATUS"]) ? $_POST["radBILLSTATUS"] : '')."'");
			
			if ($cnt_3partytcs == 0)
			{
				array_push($tran3PARTYTCSFieldArr,"ENTRYDATE");
				array_push($tran3PARTYTCSValueArr,"'".date('Y-m-d h:i:s')."'");			
				newData($tran3PARTYTCSFieldArr,$tran3PARTYTCSValueArr,LEDGER_DEBIT);		
			}
			else
			{
				$Condition = " WHERE LEDGERID='".THIRDPARTYTCSAC."' AND VOUCHERTYPE='Third Party TCS' AND SRNO='".$SRNO."' AND VOUCHERNO='".$ID."'";			
				editData($tran3PARTYTCSFieldArr,$tran3PARTYTCSValueArr,LEDGER_DEBIT,$Condition);			
			}	
		}
		else
		{
		...
		array_push($tranTCSValueArr,"'TCS In'");
		...
		if($_POST["txtTCSAMT"] > 0)
		{
			if ($cnt_tcs == 0)
			{
				$tranTCSValueArr[3] = "'".TCSIN."'";
				$tranTCSValueArr[4] = "'".round($_POST["txtTCSAMT"])."'";
				$tranTCSValueArr[11] = "'".GSTGB."'";
						
				newData($tranTCSFieldArr,$tranTCSValueArr,LEDGER_DEBIT);		
			}
			else
			{
				$Condition = " WHERE LEDGERID='".TCSIN."' AND VOUCHERTYPE='TCS In' AND SRNO='".$SRNO."' AND VOUCHERNO='".$ID."'";			
				$tranTCSValueArr[3] = "'".TCSIN."'";
				$tranTCSValueArr[4] = "'".round($_POST["txtTCSAMT"])."'";
				$tranTCSValueArr[11] = "'".GSTGB."'";
				editData($tranTCSFieldArr,$tranTCSValueArr,LEDGER_DEBIT,$Condition);			
			}	
		}
		else
		{
		...
		/*if(isset($_POST["txtBROKERID"]) &&  !empty($_POST["txtBROKERID"]) && isset($_POST["txtBROKERID"]) && $_POST["txtBROKERID"] > 0  && $_POST["txtDALALIAMT"] > 0)
		{
			...
			{
				$Condition = " WHERE VOUCHERTYPE='Purchase' AND SRNO='".$BROKERSRNO."' AND VOUCHERNO='".$ID."'";			
				editData($tranFieldArr,$tranValueArr,LEDGER_CREDIT,$Condition);
				$tranValueArr[3] = "'".BROKAC."'";
				$tranValueArr[11] = "'".BROKGBP."'";
				editData($tranFieldArr,$tranValueArr,LEDGER_DEBIT,$Condition);			
...
elseif(isset($_POST["shape_mul"]))
{
	...
	deleteData(LEDGER_DEBIT," WHERE VOUCHERNO IN ('".$DeleteString."') AND VOUCHERTYPE in ('Purchase','Third Party Charges','Tax In')");
...
{?>
			<div class="row form-group">
				<div class="col-lg-1">
						...
						{
							?>
							<a class="btn btn-success addcls" style="margin-right:10px;" href="<?php echo SITEURL; ?>?purchase&_nid"><i class="fa fa-plus-circle"></i> Add New</a>
							<?php
						}?>
				</div>
					<div class="col-lg-1">
						<a class="btn btn-primary addcls" style="margin-left:8px;" href="<?php echo SITEURL; ?>?purchase&_nid=b"><i class="fa fa-plus-circle"></i> Add Bulk</a>
					</div>
					<div class="col-lg-1">
				...
				<div class="col-lg-2">
					<?php								
						...
						{
					?>
						<a class="btn btn-primary" style="margin-left:40px;" href="<?php echo SITEURL;?>?purchase&_vid"><i class="fa fa-plus-tasks"></i>View</a>
					<?php
						}
					?>
				</div>
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?purchase&_vid" method="POST" onsubmit="">
					<div class="col-lg-2">
						Barcode No : <input type="text" class="form-control " name="search_barcode_pur" id="search_barcode_pur"/>
					</div>
					<div class="col-lg-2">
						<input style="margin-right:0px;margin-top:16px;" class="form-control" name="STOCKIDSEARCH" id="STOCKIDSEARCH" placeholder="Search">
					</div>
					<div class="col-lg-2">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?purchase" method="POST" onsubmit="return confirm('Do you really want to Delete selected Invoice?');">
					<?php echo $back_button;?>
					<p>
						
						<a class="btn btn-success addcls"  href="<?php echo SITEURL; ?>?purchase&_nid"><i class="fa fa-plus-circle"></i> Add New</a>
						<?php
						if($del_bol)
						{
							?>
						...
						<a class="btn btn-success" href="<?php echo SITEURL; ?>makexls.php?makexls=dashboard_pe_2" onclick="return confirm('Do you really want to Make XLS);" class="btn btn-success btn-circle" title="XLS">
							<i class="fa fa-file-excel-o"></i> Make XLS
													</a>	
						<a class="btn btn-primary" href="<?php echo SITEURL; ?>makexls.php?makexls=purchase_" onclick="return confirm('Do you really want to Make XLS:?');" class="btn btn-primary btn-circle" title="XLS">
				...
				<?php
							...
							if($orderBy == "VOUCHERDATE" and $order == "asc") {
							...
							if($orderBy == "BARCODENO" and $order == "asc") {
							
								$BARCODENO = "desc";
							
							}
							
							if($orderBy == "CERTIFICATENO" and $order == "desc") {
						 ...
						 <table class="table table-striped table-bordered table-hover customResponsiveTable" id="dataTables-example">
							<thead>
								<tr style="font-size:9px;">
									...
									<th style="font-size:11px"><a href="<?php echo SITEURL; ?>?purchase&_orderby=VOUCHERDATE&order=<?php echo $VOUCHERDATE; ?>" class="column-title">Date</a></th>	
									<th style="font-size:11px"><a href="<?php echo SITEURL; ?>?purchase&_orderby=BARCODENO&order=<?php echo $BARCODENO; ?>" class="column-title">Pt. St.</a></th>	
									<th style="font-size:11px">Stock Id</th>
									<th style="font-size:11px">Party</th>
									<th style="font-size:11px">Broker</th>
									
																
									<th style="font-size:11px">WT</th>	
									<th style="font-size:11px">Shp</th>	
									<th style="font-size:11px">Cl</th>	
									<th style="font-size:11px">Cal</th>	
									<th style="font-size:11px">Ct</th>	
									<th style="font-size:11px">PO</th>	
									<th style="font-size:11px">Sy</th>	
									<th style="font-size:11px">Flu</th>
									<th style="font-size:11px"><a href="<?php echo SITEURL; ?>?purchase&_orderby=CERTIFICATENO&order=<?php echo $CERTIFICATENO; ?>" class="column-title">Certi</a></th>										
									<th style="font-size:11px">Lb</th>	
									
									
									<th style="font-size:11px">Rate</th>
									<th style="font-size:11px">Disc</th>
									<th style="font-size:11px">Disc1</th>
									<th style="font-size:11px">Disc2</th>
									<th>$/Crt</th>
									<th style="font-size:11px">Rate $</th>
									<th>$</th>
									<th style="font-size:11px">Rs/Crt</th>
									<th style="font-size:11px">Rs Amt</th>
								...
								while($resdata = mysqli_fetch_assoc($res))
									{
										...
										?>
											<tr style="font-size:9px;" class="<?php echo $classname;?>">
											...
											<?php
												...
												<td><?php echo $resdata["PARTNERSTATUS"] == "Y" ? "YES".$resdata["PARTNERPER"] : "NO";?></td>
												...
												<td class="amountalign"><?php echo getCurrFormat($resdata["DISC2PER"]) ;?></td>
												<td class="amountalign"><?php echo getCurrFormat($resdata["DISC3PER"]) ;?></td>
												...
												<td>
													...
													{
														?>
													<a href="<?php echo SITEURL; ?>?purchase&_mid=<?php echo $resdata["ID"];?>" class="btn btn-primary btn-circle editcls" title="Edit">
													...
													{
														?>
													<a href="<?php echo SITEURL; ?>?purchase&_rid=<?php echo $resdata["ID"];?>" onclick="return confirm('Do you really want to Delete Purchase : <?php echo $resdata["ID"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
				...
				?>
				<b>Note:- 	<li>PT. ST. is PARTNER STATUS</li>
							<!--<li>WT is WEIGHT</li>
							<li>SHP is SHAPE</li>
							<li>CL is COLOR</li>
							<li>CAL is CLARITY</li>
							<li>PO is POLISH</li>
							<li>SY is SYMMETRY</li>
							<li>FLU is FLOURANCE</li>-->
							</b>
					</form>
				</div>
				<!-- /.panel-body -->
	...
	?>
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<?php echo $back_button;?>
					<form id="frm_Purchase" action="<?php echo SITEURL; ?>?purchase" method="POST" enctype="multipart/form-data">
						...
						<div class="row form-group">
							<div class="col-lg-6">
								<div class="checkbox-inline">
											...
											<label>
												...
												<input class="checkbox-inline clsbillstatus" type="radio" id="radthirdpartybillstatus" name="radBILLSTATUS" value="Third Party Bill" <?php echo $resdata["BILLSTATUS"] == 'Third Party Bill' || $resdata["BILLSTATUS"] == '' ? "checked" : "";?>/>Third Party Bill
							...
							<div class="row form-group GST" style="<?php echo $resdata["BILLSTATUS"] == 'With Bill' ? '' : "display:none;";?>" >
						...
						</div>	<div class="row form-group GST" style="<?php echo $resdata["BILLSTATUS"] == 'With Bill' ? '' : "display:none;";?>" >
								...
								<div class="col-lg-12">
												...
												<div class="col-lg-1"><input class="form-control GSTCAL" name="txtTCSPER" id="txtTCSPER" value="<?php echo $resdata["TCSPER"];?>"></div>
												<div class="col-lg-2"><label><input style="text-align:right;" class="form-control" name="txtTCSAMT" id="txtTCSAMT" value="<?php echo $resdata["TCSAMT"];?>">	</label></div>
											
								</div>
							</div>
											
						<div class="row form-group thirdpartycls" style="<?php echo $resdata["BILLSTATUS"] == 'Third Party Bill' ? '' : "display:none;";?>" >
								...
								<div class="col-lg-12">
												...
												<div class="col-lg-1"><label>Third Party Charges</label></div>
												<div class="col-lg-1"><input class="form-control" name="txtTHIRDPARTYCHARGESPER" id="txtTHIRDPARTYCHARGESPER" value="<?php echo $resdata["THIRDPARTYCHARGESPER"];?>"></div>
												<div class="col-lg-2"><label><input style="text-align:right;" class="form-control" name="txtTHIRDPARTYCHARGES" id="txtTHIRDPARTYCHARGES" value="<?php echo $resdata["THIRDPARTYCHARGES"];?>">	</label></div>
											
								</div>
						</div><div class="row form-group thirdpartycls" style="<?php echo $resdata["BILLSTATUS"] == 'Third Party Bill' ? '' : "display:none;";?>" >
								...
								<div class="col-lg-12">
												...
												<div class="col-lg-1"><label>Third Party TCS</label></div>
												<div class="col-lg-1"><input class="form-control" name="txtTHIRDPARTYTCSPER" id="txtTHIRDPARTYTCSPER" value="<?php echo $resdata["THIRDPARTYTCSPER"];?>"></div>
												<div class="col-lg-2"><label><input style="text-align:right;" class="form-control" name="txtTHIRDPARTYTCS" id="txtTHIRDPARTYTCS" value="<?php echo $resdata["THIRDPARTYTCS"];?>">	</label></div>
					...
					<input type="hidden"  class="form-control" readonly name="companystate" id="companystate" value="<?php echo $rescompanydata["STATE"];?>">
						...
						<input type="hidden" name="filename_str" value="purchase"/>
