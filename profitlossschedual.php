...
if(isset($_POST["print-profitlossschedual"]))
{
	
	$dtfrm = $_POST["dtpFROMDATE"];
	$dtto = $_POST["dtpENDDATE"];
//	$OPENSTOCK = getFieldDetail(BARCODE_PROCESS,"SUM(STOCKIDVALUE)"," where PROCESSTYPE='Purchase' and VDATE < '".$dtfrm."' and BARCODENO NOT IN (SELECT BARCODENO FROM ".BARCODE_PROCESS." WHERE VDATE < '".$dtfrm."' AND PROCESSTYPE='Sale')");
		
		
	
	//$COSINGSTOCK = ( getFieldDetail(BARCODE_PROCESS,"SUM(STOCKIDVALUE)"," where PROCESSTYPE='Purchase' and VDATE <= '". $dtto ."' and BARCODENO NOT IN (SELECT BARCODENO FROM ".BARCODE_PROCESS." WHERE VDATE <= '". $dtto ."' AND PROCESSTYPE='Sale')"));
	...
	array_push($drarr,"DR.LEDGERID");
	array_push($drarr,"DR.LEDGERNAME");
	array_push($drarr,"DR.AMOUNT AS AMOUNT");
	array_push($drarr,"DR.AMOUNTDOLLAR AS AMOUNTDOLLAR");
	array_push($drarr,"DR.RMBAMOUNT AS RMBAMOUNT");
	...
	array_push($crarr,"CR.LEDGERID");
	array_push($crarr,"CR.LEDGERNAME");
	array_push($crarr,"CR.AMOUNT AS AMOUNT");
	array_push($crarr,"CR.AMOUNTDOLLAR AS AMOUNTDOLLAR");
	array_push($crarr,"CR.RMBAMOUNT AS RMBAMOUNT");
	...
	while($resdata = mysqli_fetch_assoc($res_gbp))
	{
		...
		if( ($bal_arr[0] != 0 && $bal_arr[1] != '') )
		{
			...
			//==================ledger for group==================================================
			$resledger=getData(LEDGER,$AllArr," where GROUPID IN ('".$resdata["GROUPID"]."')");
			while($resledgerdata = mysqli_fetch_assoc($resledger))
			{
				
				
				
					$bal_arr = getBetweenData($dtfrm,$dtto,$resledgerdata["LEDGERID"],false);		 
					
				//if( ($bal_arr[0] != 0 && $bal_arr[1] != '') || ($dollarbal_arr[0] != 0 && $dollarbal_arr[1] != '') || ($rmbbal_arr[0] != 0 && $rmbbal_arr[1] != ''))
					if( ($bal_arr[0] != 0 && $bal_arr[1] != '') )
				{
					//===============================DR==============
					//if($bal_arr[1] == "Dr" || $dollarbal_arr[1] == "Dr" || $rmbbal_arr[1] == "Dr")
	...
	while($res_gbpdata = mysqli_fetch_assoc($res_gbp)) //======================GROUP WHILE START
	{
		...
		if( ($bal_arr[0] != 0 && $bal_arr[1] != '') )
		{
			...
			//==================ledger for group==================================================
			$resledger=getData(LEDGER,$AllArr," where GROUPID IN ('".$res_gbpdata["GROUPID"]."')");
			while($resledgerdata = mysqli_fetch_assoc($resledger))
			{
				$bal_arr = getBetweenData($dtfrm,$dtto,$resledgerdata["LEDGERID"],false);		 
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Profit & Loss Schedual</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmledgertable" id="frmledgertable" action="<?php echo SITEURL; ?>?print-profitlossschedual" method="POST">
					...
					<div class="form-group">
							...
							<div class="form-group">
								<button type="submit" class="btn btn-default" style="float: right;margin-left:10px;" name="print-profitlossschedual" id="printledger">Submit Button</button>
								<button type="submit" class="btn btn-danger" style="float: right;" name="print-profitlossschedual-pdf" id="printledgerpdf"><i class="fa fa-file-pdf-o"></i> Make PDF</button>
								<br/><br>
								</div>
						</div>
