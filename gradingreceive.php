...
$FieldArr= array();
			...
			$res = getData(BARCODE_PROCESS,$FieldArr," AS BP LEFT JOIN ".LEDGER." AS L ON L.LEDGERID=BP.LEDGERID LEFT JOIN ".LEDGER." AS B on B.LEDGERID=BP.BROKERID WHERE BP.FLAG='0' AND BP.PROCESSTYPE='Grading Receive' ORDER BY BP.ID DESC");
			$salSTRCAPTION="";
			$SALxlsaction="";		
			$xlsaction1="";	
if(isset($_GET["_nid"]))
{
	$action ="new";
	$Caption = "New Grading Receive Detail";
	$ID = $_GET["_nid"];
	
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$ID."' AND PROCESSTYPE='Grading Receive'");
...
elseif(isset($_GET["_mid"]))
{
	...
	$ID = $_GET["_mid"];
	//echo $ID ;
	$Caption = "Edit Grading Receive Detail";
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ENTRYID='".$ID."' AND PROCESSTYPE='Grading Receive'");
	$resdata = mysqli_fetch_assoc($res);	
...
elseif(isset($_GET["_rid"]))
{
	...
	$ENTRYID = $_GET["_rid"];	
	$ID = $_GET["_IDSTR"];	
	deleteData(BARCODE_PROCESS," where ENTRYID='".$ENTRYID."' AND PROCESSTYPE='Grading Receive'");
	deleteData(LEDGER_CREDIT," where VOUCHERNO='".$ID."' AND VOUCHERTYPE='Grading Receive Expense'");
	deleteData(LEDGER_DEBIT," where VOUCHERNO='".$ID."' AND VOUCHERTYPE='Grading Receive Expense'");
	?>
	<script>
		window.location.href="<?php echo SITEURL."?gradingreceive&_vid";?>";
	</script>
	<?php
}
...
if(isset($_POST["gradingreceive"]))
{
	
	$idx = 0;
	$Barcode_Arr = $_POST["BARCODENO"];
	$ID=($_POST["ID"] == 0 ? getMaxValue(BARCODE_PROCESS,"ID"," WHERE PROCESSTYPE='Grading Receive'") : $_POST["ID"]);
	foreach ($Barcode_Arr as $Barcode)
	{
		...
		$reccnt = getFieldDetail(BARCODE_PROCESS,"count(*)"," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Grading Receive'");
		...
		if ($reccnt == 0)
		{
			...
			array_push($ValueBarcode,"'Grading Receive'");
		...
		else
		{
			...
			editData($FieldBarcode,$ValueBarcode,BARCODE_PROCESS," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Grading Receive'");
		}	
		...
		//=====================ledger entry========================
			if ($EXPENCE != 0)
			{
				...
				$DRLEDGERID =3362;//MEMORECEIVE_ANG_EXPENSE_LEDGERID
				...
				$cnt_ledger_DR  = getFieldDetail(LEDGER_DEBIT,"count(*)", " WHERE VOUCHERNO = '".$ID."' AND DESCRIPTION = '".$Barcode."' AND LEDGERID = '".$DRLEDGERID. "' and VOUCHERTYPE='Grading Receive Expense'");
				$dr = $cnt_ledger_DR;
				$cnt_ledger_CR =  getFieldDetail(LEDGER_CREDIT,"count(*)"," WHERE VOUCHERNO = '" .$ID. "' AND DESCRIPTION = '" .$Barcode. "' AND LEDGERID = '".$CRLEDGERID ."' and VOUCHERTYPE='Grading Receive Expense'");
                $cr=$cnt_ledger_CR; 
				...
				{
                    $SRNO = getfielddetail(LEDGER_DEBIT, "SRNO", " WHERE VOUCHERNO = '" .$ID. "' AND DESCRIPTION = '".$Barcode."' AND LEDGERID = '".$DRLEDGERID . "' and VOUCHERTYPE='Grading Receive Expense'");
			    }
				elseif ($cnt_ledger_DR = 0 && $cnt_ledger_CR > 0 )
				{
                    $SRNO = getfielddetail(LEDGER_DEBIT, "SRNO", " WHERE VOUCHERNO = '" .$ID."' AND DESCRIPTION = '" .$Barcode."' AND LEDGERID = '".$CRLEDGERID ."' and VOUCHERTYPE='Grading Receive Expense'");
				}
				else
				{
					$SRNO = getMaxValue_TRAN(LEDGER_DEBIT, "SRNO");
					...
					array_push($tranValueArr,"'Grading Receive Expense'");					
					...
					$Condition = " WHERE VOUCHERTYPE='Grading Receive Expense' AND DESCRIPTION='".$Barcode."' AND VOUCHERNO='".$ID."'";
				 if ($dr  == 0)
				 {
	...
	?>
	<script>
		window.location.href="<?php echo SITEURL."?gradingreceive";?>";
	</script>
		<?php
		exit();
	$action = "";	
...
elseif(isset($_POST["shape_mul"]))
{
	...
	$Condition = " WHERE ID IN (".$DeleteString.") AND VOUCHERTYPE='Grading Receive'";
	
	
	deleteData(BARCODE_PROCESS," where ID IN (".$DeleteString.") AND PROCESSTYPE='Grading Receive'");
	deleteData(LEDGER_CREDIT," where VOUCHERNO IN (".$DeleteString.") AND VOUCHERTYPE='Grading Receive Expense'");
	deleteData(LEDGER_DEBIT," where VOUCHERNO IN (".$DeleteString.") AND VOUCHERTYPE='Grading Receive Expense'");
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Grading Receive</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
if($action=="")
{
						...
						{
							?>
							<a class="btn btn-success addcls" href="<?php echo SITEURL; ?>?gradingreceive&_nid"><i class="fa fa-plus-circle"></i> Add New</a>
							<?php
						}
						if($view_bol)
						{
							?>
							<a class="btn btn-primary" href="<?php echo SITEURL; ?>?gradingreceive&_vid"><i class="fa fa-plus-tasks"></i>View</a>
							<?php
						}
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?gradingreceive" method="POST" onsubmit="return confirm('Do you really want to Delete selected Invoice?');">
										...
										?>
											...
											<td><?php echo $resdata["REMARK"];?></td>
												...
												<td>
													...
													{
														?>
													<a href="<?php echo SITEURL; ?>?gradingreceive&_mid=<?php echo $resdata["ENTRYID"];?>" class="btn btn-primary btn-circle editcls" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?gradingreceive&_rid=<?php echo $resdata["ENTRYID"];?>&_IDSTR=<?php echo $resdata["ID"];?>" onclick="return confirm('Do you really want to Delete Sale : <?php echo $resdata["SHAPENAME"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
	...
	?>
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<?php echo $back_button;?>
					<form id="frm_Sale" action="<?php echo SITEURL; ?>?gradingreceive" method="POST" >
						...
						<div class="form-group">
							<table id="addmorefield" width="110%" class="inputaddmorefieldtable customResponsiveTable">
								...
								<?php
									$resprod = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$resdata["ID"]."' AND PROCESSTYPE='Grading Receive'");
									...
									while($resproddata = mysqli_fetch_assoc($resprod))
									{
										...
										<tr>
											...
											<td style="text-align:center;"><a href="javascript:void(0)" class="btn btn-danger btn-circle remove_field" rel="<?php echo $resproddata["BARCODENO"];?>/GradingReceive" ><i class="fa fa-remove"></i></a></td>
										</tr>
										<?php
									}
								?>
						...
						<button type="submit" class="btn btn-default" style="float: right;margin-left: 10px;" name="gradingreceive" id="btnSale">Submit Button</button>
