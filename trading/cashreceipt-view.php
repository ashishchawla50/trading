...
if(isset($_POST["SRNO"]) && isset($_POST["VOUCHERTYPE"]) && isset($_POST["tran-cashreceipt"]))
{
	...
	?>
	<script>
		window.location.href='<?php echo SITEURL; ?>?tran-cashreceipt-v';
	</script>
	<?php
	exit();
}
else
{
	...
	if(isset($_GET["_mid"]))
	{
		...
		$res = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Cash Receipt' AND DR.VOUCHERTYPE='Cash Receipt' AND DR.SRNO='".$SRNO."' AND CR.SRNO='".$SRNO."'");
...
//$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Cash Receipt' AND DR.VOUCHERTYPE='Cash Receipt' ORDER BY DR.VOUCHERDATE DESC,DR.SRNO ASC");
$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE WHERE CR.VOUCHERTYPE='Cash Receipt' AND DR.VOUCHERTYPE='Cash Receipt' ORDER BY DR.SRNO DESC");

?>
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Cash - Receipt - Voucher - View</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
...
<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?tran-cashreceipt-v" method="POST" onsubmit="return confirm('Do you really want to Delete?');">
					...
					<div class="dataTable_wrapper">
						<table class="table table-striped table-bordered table-hover" id="dataTables-example">
							<thead>
								<tr>
									<td colspan="5"></td>
									<td colspan="3" style="text-align:center;font-size:1.2em;"><label>Debit</label></td>
									<td colspan="3" style="text-align:center;font-size:1.2em;"><label>Credit</label></td>
									<td colspan="1"></td>
									<td colspan="1"></td>
								</tr>
								<tr>
								...
								while($resdata = mysqli_fetch_assoc($res_dr))
										{
											...
											?>
												...
												<td class="delcls">
													<a href="<?php echo SITEURL; ?>?cashreceipt-view&_mid=<?php echo $resdata["SRNO"];?>" class="btn btn-primary btn-circle" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?cashreceipt-view&_rid=<?php echo $resdata["SRNO"];?>" onclick="return confirm('Do you really want to Delete This Transaction : <?php echo $resdata["SRNO"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<p>
						<a  class="btn btn-warning" href="<?php echo SITEURL; ?>?cashreceipt-view" style="float:right;" ><i class="fa fa-tasks"></i> View All</a>
						<br>
					</p>
					<form id="frm_journaltable_edit" action="<?php echo SITEURL; ?>?tran-cashreceipt-v" method="POST" >
						...
						<button type="submit" class="btn btn-default" style="float: right;" name="tran-cashreceipt">Submit Button</button>
					</form>
				</div>
			</div>
		</div>
