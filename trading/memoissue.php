...
$FieldArr= array();
			...
			$res = getData(BARCODE_PROCESS,$FieldArr," AS BP INNER JOIN ".LEDGER." AS L ON L.LEDGERID=BP.LEDGERID LEFT JOIN ".LEDGER." AS B on B.LEDGERID=BP.BROKERID WHERE BP.FLAG='0' AND BP.PROCESSTYPE='Memo Issue' ORDER BY BP.ID DESC");
			$salSTRCAPTION="";
			$SALxlsaction="";		
			$xlsaction1="";	
if(isset($_GET["_nid"]))
{
	$action ="new";
	$Caption = "New Memo Issue Detail";
	$ID = $_GET["_nid"];
	
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$ID."' AND PROCESSTYPE='Memo Issue'");
...
elseif(isset($_GET["_mid"]))
{
	...
	$ID = $_GET["_mid"];
	//echo $ID ;
	$Caption = "Edit Memo Issue Detail";
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ENTRYID='".$ID."' AND PROCESSTYPE='Memo Issue'");
	$resdata = mysqli_fetch_assoc($res);	
...
elseif(isset($_GET["_rid"]))
{
	$action ="remove";
	$ID = $_GET["_rid"];	
	deleteData(BARCODE_PROCESS," where ENTRYID='".$ID."' AND PROCESSTYPE='Memo Issue'");
	?>
	<script>
		window.location.href="<?php echo SITEURL."?memoissue&_vid";?>";
	</script>
	<?php
}
...
if(isset($_POST["memoissue"]))
{
	$idx = 0;
	$Barcode_Arr = $_POST["BARCODENO"];
	$ID=($_POST["ID"] == 0 ? getMaxValue(BARCODE_PROCESS,"ID"," WHERE PROCESSTYPE='Memo Issue'") : $_POST["ID"]);
	...
	array_push($FieldBarcode,"RAPDISCOUNT");
		...
		$reccnt = getFieldDetail(BARCODE_PROCESS,"count(*)"," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Memo Issue'");
		...
		if ($reccnt == 0)
		{
			...
			array_push($ValueBarcode,"'Memo Issue'");
		...
		else
		{
			...
			editData($FieldBarcode,$ValueBarcode,BARCODE_PROCESS," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Memo Issue'");
		}		
	}
	...
	?>
	<script>
		window.location.href="<?php echo SITEURL."?memoissue";?>";
	</script>
		<?php
		exit();
	$action = "";	
...
elseif(isset($_POST["shape_mul"]))
{
	...
	$Condition = " WHERE ID IN (".$DeleteString.") AND VOUCHERTYPE='Memo Issue'";
	
	
	deleteData(BARCODE_PROCESS," where ID IN (".$DeleteString.") AND PROCESSTYPE='Memo Issue'");
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Memo Issue</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
if($action=="")
{
						...
						{
							?>
							<a class="btn btn-success addcls" href="<?php echo SITEURL; ?>?memoissue&_nid"><i class="fa fa-plus-circle"></i> Add New</a>
							<?php
						}
						if($view_bol)
						{
							?>
							<a class="btn btn-primary" href="<?php echo SITEURL; ?>?memoissue&_vid"><i class="fa fa-plus-tasks"></i>View</a>
							<?php
						}
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?memoissue" method="POST" onsubmit="return confirm('Do you really want to Delete selected Invoice?');">
										...
										?>
											...
											<td><?php echo $resdata["REMARK"];?></td>
												...
												<td>
													...
													{
														?>
													<a href="<?php echo SITEURL; ?>?memoissue&_mid=<?php echo $resdata["ENTRYID"];?>" class="btn btn-primary btn-circle editcls" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?memoissue&_rid=<?php echo $resdata["ENTRYID"];?>" onclick="return confirm('Do you really want to Delete Sale : <?php echo $resdata["SHAPENAME"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
	...
	?>
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<?php echo $back_button;?>
					<form id="frm_Sale" action="<?php echo SITEURL; ?>?memoissue" method="POST" >
						...
						<div class="form-group">
							<table id="addmorefield" width="110%" class="inputaddmorefieldtable customResponsiveTable">
								...
								<?php
									$resprod = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$resdata["ID"]."' AND PROCESSTYPE='Memo Issue'");
						...
						<button type="submit" class="btn btn-default" style="float: right;margin-left: 10px;" name="memoissue" id="btnSale">Submit Button</button>
