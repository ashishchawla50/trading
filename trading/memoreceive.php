...
$FieldArr= array();
			...
			$res = getData(BARCODE_PROCESS,$FieldArr," AS BP INNER JOIN ".LEDGER." AS L ON L.LEDGERID=BP.LEDGERID LEFT JOIN ".LEDGER." AS B on B.LEDGERID=BP.BROKERID WHERE BP.FLAG='0' AND BP.PROCESSTYPE='Memo Receive' ORDER BY BP.ID DESC");
			$salSTRCAPTION="";
			$SALxlsaction="";		
			$xlsaction1="";	
if(isset($_GET["_nid"]))
{
	$action ="new";
	$Caption = "New Memo Receive Detail";
	$ID = $_GET["_nid"];
	
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$ID."' AND PROCESSTYPE='Memo Receive'");
...
elseif(isset($_GET["_mid"]))
{
	...
	$ID = $_GET["_mid"];
	//echo $ID ;
	$Caption = "Edit Memo Receive Detail";
	$res = getData(BARCODE_PROCESS,$AllArr," WHERE ENTRYID='".$ID."' AND PROCESSTYPE='Memo Receive'");
	$resdata = mysqli_fetch_assoc($res);	
...
elseif(isset($_GET["_rid"]))
{
	$action ="remove";
	$ID = $_GET["_rid"];	
	deleteData(BARCODE_PROCESS," where ENTRYID='".$ID."' AND PROCESSTYPE='Memo Receive'");
	?>
	<script>
		window.location.href="<?php echo SITEURL."?memoreceive&_vid";?>";
	</script>
	<?php
}
...
if(isset($_POST["memoreceive"]))
{
	$idx = 0;
	$Barcode_Arr = $_POST["BARCODENO"];
	$ID=($_POST["ID"] == 0 ? getMaxValue(BARCODE_PROCESS,"ID"," WHERE PROCESSTYPE='Memo Receive'") : $_POST["ID"]);
	...
	array_push($FieldBarcode,"RAPDISCOUNT");
		...
		$reccnt = getFieldDetail(BARCODE_PROCESS,"count(*)"," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Memo Receive'");
		...
		if ($reccnt == 0)
		{
			...
			array_push($ValueBarcode,"'Memo Receive'");
		...
		else
		{
			...
			editData($FieldBarcode,$ValueBarcode,BARCODE_PROCESS," WHERE ENTRYID = '".$_POST["ENTRYID".$cnt_idx]."' AND ID='". $_POST["ID"] ."' AND BARCODENO='".$Barcode."' AND PROCESSTYPE='Memo Receive'");
		}		
	}
	...
	?>
	<script>
		window.location.href="<?php echo SITEURL."?memoreceive";?>";
	</script>
		<?php
		exit();
	$action = "";	
...
elseif(isset($_POST["shape_mul"]))
{
	...
	$Condition = " WHERE ID IN (".$DeleteString.") AND VOUCHERTYPE='Memo Receive'";
	
	
	deleteData(BARCODE_PROCESS," where ID IN (".$DeleteString.") AND PROCESSTYPE='Memo Receive'");
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Memo Receive</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
if($action=="")
{
						...
						{
							?>
							<a class="btn btn-success addcls" href="<?php echo SITEURL; ?>?memoreceive&_nid"><i class="fa fa-plus-circle"></i> Add New</a>
							<?php
						}
						if($view_bol)
						{
							?>
							<a class="btn btn-primary" href="<?php echo SITEURL; ?>?memoreceive&_vid"><i class="fa fa-plus-tasks"></i>View</a>
							<?php
						}
	...
	<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?memoreceive" method="POST" onsubmit="return confirm('Do you really want to Delete selected Invoice?');">
										...
										?>
											...
											<td><?php echo $resdata["REMARK"];?></td>
												...
												<td>
													...
													{
														?>
													<a href="<?php echo SITEURL; ?>?memoreceive&_mid=<?php echo $resdata["ENTRYID"];?>" class="btn btn-primary btn-circle editcls" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?memoreceive&_rid=<?php echo $resdata["ENTRYID"];?>" onclick="return confirm('Do you really want to Delete Sale : <?php echo $resdata["SHAPENAME"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
	...
	?>
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					<?php echo $back_button;?>
					<form id="frm_Sale" action="<?php echo SITEURL; ?>?memoreceive" method="POST" >
						...
						<div class="row form-group">
							...
							<div class="col-lg-1">
									<label>$</label>
									<input type="text" class="form-control onlyNumber" name="txtCONVRATE" id="txtCONVRATE" value="<?php echo $resdata["CONVRATE"] == 0 ? 1 : $resdata["CONVRATE"];?>">
									<p class="help-block"></p>
							</div>
						...
						<div class="form-group">
							<table id="addmorefield" width="110%" class="inputaddmorefieldtable customResponsiveTable">
								...
								<?php
									$resprod = getData(BARCODE_PROCESS,$AllArr," WHERE ID='".$resdata["ID"]."' AND PROCESSTYPE='Memo Receive'");
									...
									while($resproddata = mysqli_fetch_assoc($resprod))
									{
										...
										<tr>
											...
											<td style="text-align:center;"><a href="javascript:void(0)" class="btn btn-danger btn-circle remove_field" rel="<?php echo $resproddata["BARCODENO"];?>/MemoReceive" ><i class="fa fa-remove"></i></a></td>
										</tr>
										<?php
									}
								?>
						...
						<button type="submit" class="btn btn-default" style="float: right;margin-left: 10px;" name="memoreceive" id="btnSale">Submit Button</button>
