...
if(isset($_FILES["Upload_file_sale"]))
{
	

	$XLSPATH = isset($_FILES["Upload_file_sale"]["tmp_name"]) ? $_FILES["Upload_file_sale"]["tmp_name"] : "";
	$Path = UPLOAD."xls/SALE/";
	$XLSPATH = $Path."SALE.xls";
	move_uploaded_file($_FILES["Upload_file_sale"]["tmp_name"],$XLSPATH);
	...
	for($i=2;$i<=count($row);$i++)
	{
		...
		$DALALIPER_pur= getFieldDetail(PURCHASESALE,"DALALIPER"," WHERE ID in (SELECT ID FROM ".BARCODE_PROCESS." WHERE PROCESSTYPE='Purchase' AND BARCODENO='".$BARCODENO."') AND VOUCHERTYPE='Purchase'"); 
		
		$PURAMOUNT= getFieldDetail(BARCODE_PROCESS,"RSAMOUNT"," WHERE BARCODENO='". $BARCODENO ."' and PROCESSTYPE='Purchase'"); 
		...
		<tr>
			...
			<td>
				<input type="text" class="form-control onlyNumber" name="PCS<?php echo $last_barcodeno;?>" id="PCS<?php echo $last_barcodeno;?>" >
			</td>
			<td>
			...
			<td class="RMB" style= "display:none;">
			...
			<td>
				<input type="text" class="form-control DIFFAMOUNT_" name="DIFFAMOUNT<?php echo $last_barcodeno;?>" id="DIFFAMOUNT<?php echo $last_barcodeno;?>" rel="<?php echo $last_barcodeno;?>"  value="<?php echo round($DIFFAMOUNT,2) ;?>">
			<td>
				<input type="text" class="form-control" name="BGM<?php echo $last_barcodeno;?>" id="BGM<?php echo $last_barcodeno;?>"  rel="<?php echo $last_barcodeno;?>" value="<?php echo round($BGM,2) ;?>">
			</td>
