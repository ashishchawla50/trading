<?php
	...
	//$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE   WHERE CR.VOUCHERTYPE='Journal Receipt' AND DR.VOUCHERTYPE='Journal Receipt' ORDER BY DR.VOUCHERDATE DESC,DR.SRNO ASC");
	$res_dr = getData(LEDGER_DEBIT,$DebitCreditArr,"  AS DR INNER JOIN ".LEDGER_CREDIT." AS CR ON CR.SRNO=DR.SRNO AND CR.VOUCHERTYPE=DR.VOUCHERTYPE   WHERE CR.VOUCHERTYPE='Journal Receipt' AND DR.VOUCHERTYPE='Journal Receipt' ORDER BY  DR.SRNO DESC");
...
if(isset($_GET["_mid"]))
{
	...
	$res_dredit = getData(LEDGER_DEBIT,$AllArr," WHERE SRNO='".$ID."' AND VOUCHERTYPE='Journal Receipt'");
	$resdataedit = mysqli_fetch_assoc($res_dredit);	
	$res_cr = getData(LEDGER_CREDIT,$AllArr," WHERE SRNO='".$ID."' AND VOUCHERTYPE='Journal Receipt'");
...
elseif(isset($_GET["_rid"]))
{
	...
	$deletecommsrno = getFieldDetail(LEDGER_DEBIT,"COMMISIONSRNO"," WHERE SRNO ='".$ID."' AND VOUCHERTYPE='Journal Receipt'");
	//exit();
	deleteData(LEDGER_DEBIT," WHERE SRNO ='".$ID."' AND VOUCHERTYPE='Journal Receipt'");
	deleteData(LEDGER_DEBIT," WHERE SRNO ='".$deletecommsrno."' AND VOUCHERTYPE='Journal Receipt'");
	
	deleteData(LEDGER_CREDIT," WHERE SRNO ='".$ID."' AND VOUCHERTYPE='Journal Receipt'");
	deleteData(LEDGER_CREDIT," WHERE SRNO ='".$deletecommsrno."' AND VOUCHERTYPE='Journal Receipt'");
	
	?>
	<script>
		window.location.href='<?php echo SITEURL; ?>?tran-journalreceipt-v';
	</script>
	<?php
}
else
...
if(isset($_POST["tran-journalreceipt"]))
{
			...
			editData($tranFieldArr,$tranValueArr,LEDGER_CREDIT," WHERE SRNO='". $SRNO."' AND VOUCHERTYPE='Journal Receipt'");
			
			$tranValueArr[2] = $_POST["txtBOOKLEDGERID"];
			$tranValueArr[3] = getFieldDetail(LEDGER,"GROUPID"," WHERE LEDGERID='".$_POST["txtBOOKLEDGERID"]."'");
			
			editData($tranFieldArr,$tranValueArr,LEDGER_DEBIT," WHERE SRNO='". $SRNO."' AND VOUCHERTYPE='Journal Receipt'");
			...
			if($COMMISIONSRNO == '')
			{
					$COMMISIONSRNO = getMaxValue(LEDGER_CREDIT,"SRNO");
...
<div class="row">
	<div class="col-lg-12">
		<h1 class="page-header">Journal - Receipt - Voucher - View</h1>
	</div>
	 <!-- /.col-lg-12 -->
</div>
<?php
...
<div class="row">
		<div class="col-lg-12">
			<div class="panel panel-primary">
				 ...
				 <form name="frmacgrouptable" action="<?php echo SITEURL; ?>?tran-journalreceipt-v" method="POST" onsubmit="return confirm('Do you really want to Delete?');">
									...
									//exit();
										{
											...
											?>
												...
												<td class="delcls">
													...
													{
													?>
													<a href="<?php echo SITEURL; ?>?journalreceipt-view&_mid=<?php echo $resdata["SRNO"];?>" class="btn btn-primary btn-circle" title="Edit">
													...
													{	
													?>
													<a href="<?php echo SITEURL; ?>?journalreceipt-view&_rid=<?php echo $resdata["SRNO"];?>" onclick="return confirm('Do you really want to Delete This Transaction : <?php echo $resdata["SRNO"];?>?');" class="btn btn-danger btn-circle delcls" title="Delete">
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					...
					<form id="frm_journaltable_edit" action="<?php echo SITEURL; ?>?tran-journalreceipt-v" method="POST" >
											...
											while($res_led_data = mysqli_fetch_assoc($res_led))
												{
													...
													<option value="<?php echo $res_led_data["LEDGERID"];?>" <?php echo $rescrdata["LEDGERID"] == $res_led_data["LEDGERID"] ? 'selected="selected"' : '' ?>><?php echo $res_led_data["LEDGERNAME"];?></option><?php
												}
											?>
						...
						<div class="form-group">
							<table  class="inputfieldtable">
								...
								<tr>
									<!--<td class="TRAN_USD">
										<input type="text" class="form-control onlyNumber amtchange" name="txtAMOUNTDOLLAR" id="txtDRAMOUNTDOLLAR" value="<?php echo $resdataedit["AMOUNTDOLLAR"];?>">
									</td>
									...
									<td class="TRAN_INR">
										<input type="text" class="form-control onlyNumber  CRDRAMOUNT" name="txtDRAMOUNT" id="txtDRAMOUNT" value="<?php echo $resdataedit["AMOUNT"];?>">
									</td>
									<td class="">
